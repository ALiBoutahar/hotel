    ...
    <div class="">
        ...
        <form class="d-flex flex-column" action="{{ url('chambers/'. $chambers->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group mb-2">
                ...
                <textarea class="form-control" placeholder="Laissez un message ici" required id="Description" name="Description" style="height: 80px">{{$chambers->Description}}</textarea>
                @error('Description')
                    <div class="text-danger">
                        {{$message}}
                    </div>
                ...
                <label for="images">images:</label>
                    ...
                    <div class="mt-1">
                        ...
                        @for ($i = 0; $i < count($data); $i++)
                            <img src="{{asset('storage/'.$data[$i])}}" width="70px">
                        @endfor
                    </div>
                    @error('images')
                    <div class="text-danger">
